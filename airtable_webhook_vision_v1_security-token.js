...
try {
    ...
    if (response.ok) {
        ...
        if (result.success) {
            ...
            // Server has processed the file and uploaded it to Airtable
            ...
            if (result.file_attachment) {
                output.set('file_attachment', result.file_attachment);  // AI can process this file directly
                console.log('File attachment ready for AI processing:', result.file_attachment[0].filename);
            }
            
            output.set('file_size', result.file_size);
            output.set('mime_type', result.mime_type);
            output.set('processing_status', result.processing_status || 'File processed and ready for AI analysis');
            
            if (result.ocr_failed) {
                console.log('OCR failed but file uploaded successfully for AI processing');
            } else {
                console.log('Text extraction and file upload completed successfully');
            }
        } else {
