...
@app.route('/check-file-permissions', methods=['POST'])
def check_file_permissions():
    """Check service account permissions for a file without modifying it"""
    try:
        # Validate Bearer token
        auth_header = request.headers.get('Authorization')
        if not validate_bearer_token(auth_header):
            logger.warning("Invalid bearer token for /check-file-permissions")
            return jsonify({"error": "Unauthorized"}), 401
        ...
        try:
            ...
            for perm in permissions.get('permissions', []):
                ...
                if 'drive-airtable@' in email:
                    service_email = email
                    service_role = perm.get('role')
                    break
            ...
            return jsonify({
                ...
                "service_account_role": service_role,
                "can_read": True,
                "can_rename": service_role in ['writer', 'editor', 'owner'],
                "can_trash": service_role in ['writer', 'editor', 'owner'], 
                "can_delete": service_role in ['owner'],
            ...
            if '403' in error_str or 'insufficient permissions' in error_str:
                return jsonify({
                    "file_id": file_id,
                    "accessible": False,
                    "error": "Service account has no access to this file",
                    "suggestion": "Share file/folder with: drive-airtable@drive-airtable-464711.iam.gserviceaccount.com"
                })
            else:
                return jsonify({"error": str(e)}), 500
        
    except Exception as e:
        logger.error(f"Error in /check-file-permissions: {str(e)}")
        return jsonify({"error": str(e)}), 500
